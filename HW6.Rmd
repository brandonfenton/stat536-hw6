...
Above is a plot of 1000 permuted slopes of $\beta_{year},$ with the observed slope plotted in red. In making the permutation distribution of $\beta_{year}$ we assume that there is no linear relationship between the proportion of days sampled that were below freezing and the year. Using the permutation distribution, we find that $\frac{3}{1000}$ permuted slopes are as or more extreme than the observed slope. Due to the violation of Normality, as seen in the previous diagnostic plots, the permutation test is a safer option, however the test does not alleviate the violation of constant variance. As seen in the above beanplots, the shape of the distribution of proportions varies largely from year to year. 
...
permute_function <- function(x){
  ...
  p_value <- pdata(abs(Tstar), abs(Tobs), lower.tail = F)
  print(p_value)
  # Just returning the p-value right now for diagnostic purposes
  # decision <- ifelse(p_value < 0.05, 1, 0)
  return(p_value)
}
...
clusterExport(clust, c("Data1", "prop_lm", "shuffle", "pdata","Tstar", "mblm"))
...
The presence of autocorrelation in the simulated data made drastic differences in the type I error rates of all the methods. The type I errors of the `mblm` function have become even more substatial with the presence of autocorrelation, going from 30\% to nearly 60\%. The expected error rates of the t-test for uncorrelated data, became quite large for data with autocorrelation present, going from 5\% to 31\%. Finally, the permutation method saw the smallest increase in type I error rates when correlation is present, going from  \textbf{rates here!}.   
