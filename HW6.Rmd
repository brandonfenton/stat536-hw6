...
mblm_function <- function(x){
  z <- as.vector(t(x))
  model <- mblm(z ~ Data1$Year, repeated = F)
  CI <- confint(model)
  decision <- ifelse(CI[1] < 0 & CI[2] > 0, 0, 1)
  return(decision)
} 

mblm_type_1 <- apply(sim_data[,1:4], 2, mblm_function)
...
ar_sim_data <- matrix(replicate(250, arima.sim(n = 109, 
                                            model = list( ar = c(0.6)), 
                                            sd = sigma.est)),
                             nrow = 109, ncol = 250)
...
plot(PropDays ~ Year, data = Data1, type="l", col = "blue", lwd = 2,
     ylab = "Proportion of Measured Days \nBelow 32 degrees")
lines(prop_lm$coef[1] + prop_lm$coef[2]*Year, data = Data1, lwd = 2, 
      col = "red")
lines(model1s$coef[1] + model1s$coef[2]*Year, data = Data1, lwd = 2, 
      col = "green")
lines(model_zyp$coef[1] + model_zyp$coef[2]*Year, data = Data1, lwd = 2, 
      col = "black")
```
