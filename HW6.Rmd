...
In the model fit, the estimated proportion of measured days below zero decreases between 0.0005 to 0.0001 (95\% CI) for every one year increase. There is very strong evidence for a linear relationship between year and proportion of measured days below freezing (t-stat = -3.173 $\sim t_{108},$ p-value = 0.00197). The validity of this test relies on the diagnostics, which we see pictured above. The assumption of constant variance is severely violated, as the residuals verses fitted values has a sinusoidal trend. The Normal Q-Q plot also raises some concern, as there are substantial deviations from Normality as early as the first quantile. 
...
t_type_1 <- apply(sim_data[, -1], 2, t_function)
...
mblm_type_1 <- apply(sim_data[ , 2:14], 2, mblm_function)
